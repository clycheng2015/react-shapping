...
export default class Address extends React.Component {
    ...
    render() {
        ...
        console.log(data)
        return (
            <div className="ads-container" style={{minHeight: document.documentElement.clientHeight, background: "#f3f3f1"}}>
                ...
                {data && data.length > 0 ?
                        <div><div className="list-info">
                            {data.map((i, key) => (
                                    <div key={key} className="item">
                                        <div onClick={()=>{history.push({pathname: "/newAds", state: {data: i}})}}>
                                            <div className="head">
                                        ...
                                        <div className="btn">
                                            <Radio className="my-radio" checked={i.isdefault} value={i.id} onChange={e => this._updateDefault(i.id)}>
                                                <span className="name">默认地址</span>
                                            </Radio>
                                            <span className="right">
                                            <Link className="edit-info" to={{pathname: "/newAds", state: {data: i}}}>
                                         <img src={require('static/image/ic_edit_address.png')} alt="" className="edit-img"/>编辑
                                            </Link>
                                            <span onClick={() => this._delAddress(i.id)}><img src={require('static/image/ic_delte_address.png')} alt="" className="del-img"/>删除</span>
                                        </span>
                                        </div>
                                    </div>
                                ))
                        ...
                        : <div className="empty-info" style={{height: document.documentElement.clientHeight - 130, background: "#f3f3f1"}}>
                            <img src={require('static/images/empty/noads_icon.png')} alt=""/><p> 您还没有添加收货地址</p>
                            <p onClick={() => {history.push("/newAds")}}> 赶紧去添加一个</p>
                        </div>
                }
            </div>)}
