...
export default class Special extends React.Component{
    ...
    componentDidMount(){
        // this.props.feachAndroid({pagesize:9,pagenum:2})
    }
    ...
    render(){
        ...
        if(data && data.datalist&& data.datalist.length>0){
            data.datalist.map(function (i,index) {
                if(index<8){
                    goodsList.push(i)
                }
            })
        ...
        return(
            <div className='new-special'>
                <div className='nav-tab'>
                    ...
                    {/*<p className='nav-back' onClick={()=>history.goBack()}><img src={require('static/image/exclusive_back.png')} alt=""/></p>*/}
               ...
               <div className='list'>
                   {this.titleList.map((item,key)=>  {
                       return(
                           <div className='one' key={key}>
                               <p className='title'>{item}</p>
                               <ul>
                                   {this.goods[key].map((i,index)=> {
                                       return(
                                           <li key={index} onClick={()=>{
                                               history.push(`/goodsDetail/${i.id}`)
                                           }}>
                                               <div className='image'><img src={i.bigpic} alt=""  /></div>
                                               <div className='box'>
