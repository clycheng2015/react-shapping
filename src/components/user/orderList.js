...
class OrderList extends React.Component {
    ...
    render() {
        const {list, history, hasMore, isFetching, refresh, orderState} = this.props
                    ...
                    <div className="top-box">
                        <div className="box">
                            <div className="odd-info">
                                ...
                                {rowData.state === 3 && <span className="state">待收货</span>}
                                {rowData.state === 4 && <span className="state">已完成</span>}
                                {rowData.state === 5 && <span className="state">申请退款</span>}
                                {rowData.state === 6 && <span className="state">退款成功</span>}
                                {rowData.state === 7 && <span className="state">已完成</span>}
                                {rowData.state === 8 && <span className="state">退款成功</span>}
                            </div>

                            <div onClick={() => this._goDetail(rowData.state, rowData) }>
                            ...
                            {
                                rowData.leftTime >= 0 && rowData.state === 1 &&
                                    ...
                                    <div className="btn">
                                        ...
                                        <a href={`tel:${rowData.goodsitems.telnum}`}
                                           style={{textDecoration: "none"}}> <span
                                            className="cancel">联系客服</span></a>
                                        <span className="pay"
                                              onClick={() => this._comfirmOrder(rowData.id)}>确认收货</span>
                                    </div>


                                }
                                {
                                    (rowData.state === 4 || rowData.state === 5 || rowData.state === 6 || rowData.state === 7 || rowData.state === 8) &&
                                    <div className="btn">
                                                <span
                                                    className="countinfo">总额: ￥<span>{this._sum(rowData)}</span></span>
                                        <a href={`tel:${rowData.goodsitems.telnum}`}
                                           style={{textDecoration: "none"}}> <span
                                            className="pay">联系客服</span></a>
                                    </div>
